...
void CelesteMoveset::PreProcessMovement(void* pPlayer, CMoveData* pMove) {
    ...
    if (nightmarePlugin.GetMode() != Celeste) return;
...
void CelesteMoveset::UpdateModeParams() {
    nightmarePlugin.SetModeParam(DashesLeft, dashesLeft);
    nightmarePlugin.SetModeParam(StaminaLeft, climbStamina);
    if (!nightmarePlugin.GetModeParam(InitialValue)) {
        nightmarePlugin.SetModeParam(MaxDashes, maxDashes);
        nightmarePlugin.SetModeParam(MaxStamina, wallClimbMaxStamina);
        nightmarePlugin.SetModeParam(InitialValue, 1);
    }
    maxDashes = nightmarePlugin.GetModeParam(MaxDashes);
    wallClimbMaxStamina = nightmarePlugin.GetModeParam(MaxStamina);
    nightmarePlugin.SetModeParam(Dashing, (dashing > 0) ? 1 : 0);
}
...
void CelesteMoveset::ProcessMovementDashing(void* pPlayer, CMoveData* pMove, float dt) {
    ...
    if (dashRequested > 0) {
        ...
        if (dashesLeft>0 && dashing==0 && dashingCooldown==0 && health>0) {
            ...
            dir.z = -dir.x * std::sin(pitch);
            dir.x *= std::cos(pitch);

            //applying yaw rotation
            Vector newDir{
                static_cast<float>(dir.x * std::cos(yaw) + dir.y * std::sin(yaw)),
                static_cast<float>(-dir.y * std::cos(yaw) + dir.x * std::sin(yaw)), dir.z
            };
            ...
            //nightmarePlugin.modeParams[DashRequested] = true;
...
bool CelesteMoveset::IsPlaceSuitableForWallgrab(void * player, Vector pos, float angle, Vector * placeNormal) {
    ...
    /*if (holdingUse) {
        ...
        nightmarePlugin.ServerCommand(buf);
...
void IN_DashDown(const CCommand& args) {
    if(nightmarePlugin.GetMode()==Celeste)celesteMoveset.Dash();
}
